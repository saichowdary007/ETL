...
class MSSQLAdapter {
    async connect(config) {
        this.pool = await mssql_1.default.connect({ ...config, options: { enableArithAbort: true, ...(config.options || {}) } });
        return this;
    }
    createQueryStream(query, params = []) {
        const request = new mssql_1.default.Request(this.pool);
        request.stream = true;
        params.forEach((p, i) => request.input(`p${i}`, p));
        const out = new stream_1.Readable({ objectMode: true, read() { } });
    ...
    async getPrimaryKeys() {
      ...
      ON TC.CONSTRAINT_TYPE = 'PRIMARY KEY' AND
        ...
        return result.recordset.map((r) => ({ table: r.TABLE_NAME, column: r.COLUMN_NAME }));
    }
